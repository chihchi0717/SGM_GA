...
class GA(object):
    ...
    def generate(self, max_angle, max_side):
        ...
        if load == 1:
            ...
            while pass_num<self.pop_size:
                ...
                success, x_, y_ = draw_.draw_(temp_, self.changeable_side, self.changeable_angle, -1, -1, self.mode, 1) # 畫圖後判定建模可行性, 1 for debug
    ...
    def get_fitness(self, generation, success_num, mode):
        ...
        for population_number in range(self.pop_size):
            ...
            success, x__, y__ = draw_.draw_(sid_ang, self.changeable_side, self.changeable_angle, generation, population_number, mode, 0) # 進行建模可行性判定
