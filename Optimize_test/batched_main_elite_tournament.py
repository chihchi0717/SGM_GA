...
for g in range(N_GENERATIONS):
    ...
    for i in range(POP_SIZE):
        folder = os.path.join(save_root, f"P{i+1}")
        try:
            fitness = evaluate_fitness(folder, pop[i])  # ✅ 正確地對應到每一個個體
        except Exception as e:
            print(f"⚠️ P{i+1} 評估錯誤: {e}")
            fitness = 0.01
        fitness_values.append(fitness)
        print(f"P{i+1} fitness: {fitness:.2f} pop[i]: {pop[i]}")
    ...
    # === 選擇 ===
    selected = tournament_selection(pop, fitness_values, k=3)  # k 可調整大小
