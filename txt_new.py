...
def evaluate_fitness(
    ...
    PROC_MIN, PROC_MAX = 0.0, 2.0
    UNI_MIN, UNI_MAX = 0.0, 1.0

    # === Step 2: 標準化 efficiency ===
    eff_norm = (efficiency - EFF_MIN) / (EFF_MAX - EFF_MIN)
    eff_norm = np.clip(eff_norm, 0, 1)

    # === Step 3: 標準化 process_score（取反越小越好） ===
    proc_norm = (process_score - PROC_MIN) / (PROC_MAX - PROC_MIN)
    proc_norm = 1 - np.clip(proc_norm, 0, 1)

    # === Step 4: 標準化 uniformity ===
    uni_norm = (uniformity - UNI_MIN) / (UNI_MAX - UNI_MIN)
    ...
    # === Step 6: 顯示 log（可選） ===
    print(
        f"eff_norm={eff_norm:.3f}, proc_norm={proc_norm:.3f}, uni_norm={uni_norm:.3f} → fitness={fitness:.4f}"
    )
